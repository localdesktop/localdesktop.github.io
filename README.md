...
`scripts/build-docs.sh` builds offline-readable PDFs into `manuals/` (gitignored). It
produces two manuals, selected by the first word:

```bash
bash scripts/build-docs.sh                 # Developer Manual (default)
bash scripts/build-docs.sh user            # User Manual
bash scripts/build-docs.sh all             # purge manuals/ + rebuild every variant
```

- **Developer Manual** — everything: this README, the full `gh-pages` user + developer
  guides and blog, and an architecture walkthrough of the code. Book-like (Cardo serif,
  parts). → `manuals/Local Desktop - Developer Manual.pdf`
...
- **`compact`** — a near-square 130×150mm page with small margins, sized to read on a
  foldable's inner screen (mirrors Djinn's `compact`). Applies to either manual.
...
```bash
bash scripts/build-docs.sh callgraph compact   # developer, call-graph, compact page
bash scripts/build-docs.sh user dark           # user manual, dark theme
bash scripts/build-docs.sh user dark compact   # user manual, dark, compact page
```
...
- **`callgraph`** — generated by actually walking the call graph from the seed functions
  (`scripts/docs.py`, see `CALLGRAPH_SEEDS`). Exhaustive but mechanical; being lexical and
  intra-crate, it can't see trait/`dyn` dispatch and resolves same-name methods heuristically.

Everything renders with the local `pandoc` + `xelatex` if present, falling back to a
Dockerized `pandoc` otherwise.
